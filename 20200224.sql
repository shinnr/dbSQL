...
--위의 두개의 sql은 검색하고자  하는 부서번호만 다르고 나머지 텍스트는 동일하다
--하지만 부서번호가 다르기 때문에 DBMS입장에서는 서로 다른 SQL로 인식된다
--==>다른 SQL실행 계획을 세운다
--==> 실행계획을 공유하지 못한다
--==>해결책 : 바인드 변수
--SQL에서 변경되는 부분을 별도로 전송을 하고 실행계획이 세워진 이후에 바인딩 박업을 거쳐
...
--SQL커서를 통해 개발자가 직접 데이터를 FETCH 함으로써 SELECT 결과를
--전부 불러오지 않고도 개발이 가능
...
--java의 향상된 for문과 유사
--FOR record_name(한행의 정보를 담을 변수이름 / 직접 선언안함) IN 커서이름 LOOP
--    record_name.컬럼명
--END LOOP
...
BEGIN
    ...
    --DT테이블에는 8행이 있는데 1~7번 행까지만 LOOP를 시행
